...
const FINAL_STATES = ['completed', 'cancelled', 'failed', 'rejected', 'entered-in-error'] as const;
...
export function getTaskDueSoonSchedulingReason(task: Task | undefined): TaskDueSoonCheck {
  ...
  if (task.status && FINAL_STATES.includes(task.status as any)) {
...
export function getValidTaskDueDate(task: Task): Date {
  ...
  if (Number.isNaN(dueDate.getTime())) {
    // eslint-disable-next-line no-console
    console.error('[scheduleTaskDueSoonEmail] Task has invalid due date:', raw);
...
export function parseOwnerReference(
  ...
  if (!resourceType || !id) {
    // eslint-disable-next-line no-console
    console.error('[scheduleTaskDueSoonEmail] Invalid referenceString components:', referenceString);
    return null;
  }
...
export function computeReminderTime(dueDate: Date, hoursBefore: number): Date | null {
  ...
  if (sendAfter <= new Date()) {
    // eslint-disable-next-line no-console
    console.log('[scheduleTaskDueSoonEmail] Reminder time is in the past or within window, skipping');
    return null;
  }
  return sendAfter;
}
